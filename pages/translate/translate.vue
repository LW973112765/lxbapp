	...
	<view ref="box" class="main">
		<view class="content" v-for="(item, index) in chatlist" :key="index">
			...
			<view v-else class="item item-left" @click="changeen(item.isclick)">
				...
				<!-- 文本 -->
				<view v-if="item.type=='text'" class="bubble bubble-left" v-html="item.content.replace(/\n/g,'<br/>')">
				</view>
				<!-- 图片 -->
	...
	export default {
		...
		methods: {
					...
					success: (res) => {
						...
						if (res.data.error_code) {
							...
							setTimeout(() => {
								this.chatlist.splice(-1, 1)
								this.chatlist.push({
									content: `<view>${contentcopy}的${toname}翻译结果为：\n</view><view style="color:limegreen">${res.data.trans_result[0].dst}</view>`,
									user: 'ai',
									type: 'text'
								})
								this.issend = !this.issend
			...
			chathelp() {
				...
				this.chatlist.push({
					content: `<view><view class="header" style="width: 750rpx;display: flex;justify-content: center;align-items: center;font-size: 50rpx;">郎小宝の机器人使用帮助：</view><view class="item" style="padding: 0 20rpx;margin-top: 20rpx;"><h3>一、聊天模式</h3><view class="">聊天模式分为：<text style="color: red;">闲聊模式</text>、<text style="color: red;">翻译模式</text></view></view><view class="item" style="padding: 0 20rpx;margin-top: 20rpx;"><h3>二、聊天功能</h3><view class="">聊天功能包含：<text style="color: red;">获取随机图片</text>、<text style="color: red;">获取随机视频</text>、<text style="color: red;">获取随机句子</text>、<text style="color: red;">获取随机音乐</text>、<text style="color: red;">获取当前城市天气</text>、<text style="color: red;">讲笑话</text>、<text style="color: red;">翻译文档</text>、<text style="color: red;">与机器人瞎聊等</text></view><view class="">
(注意：翻译模式下只能处理翻译请求，同样的闲聊模式下不能进行翻译)</view></view><view class="item" style="padding: 0 20rpx;margin-top: 20rpx;"><h3>三、聊天命令</h3><view class="">(蓝色数字代表该命令的快捷操作)</view><view class="">\n1.切换模式为闲聊：<text style="color: red;">切换为闲聊</text><text style="color: limegreen;">(1)</text></view><view class="">\n2.切换模式为翻译：<text style="color: red;">切换为翻译。</text><text style="color: limegreen;">(2)</text></view><view class="">\n3.获取随机图片：<text style="color: red;">图片、照片</text><text style="color: limegreen;">(3)</text></view><view class="">\n4.获取随机视频：<text style="color: red;">视频</text><text style="color: limegreen;">(4)</text></view><view class="">\n5.获取随机音乐：<text style="color: red;">音乐</text><text style="color: limegreen;">(5)</text></view><view class="">\n6.暂停音乐：<text style="color: red;">暂停音乐</text><text style="color: limegreen;">(6)</text></view><view class="">\n7.播放音乐：<text style="color: red;">暂停音乐</text><text style="color: limegreen;">(7)</text></view><view class="">
8.讲笑话：<text style="color: red;">笑话</text><text style="color: limegreen;">(8)</text></view></view></view>`,
					user: 'ai',
					type: 'text'
				})
				this.issend = !this.issend
			...
			sendcontent() {
				...
				if (this.chattype == 1) {
					//获取随机图片
					if (this.content == '图片' || this.content == '照片' || this.content == '3') {
						this.getrandomimage()
					}
					//获取随机视频
					else if (this.content == '视频' || this.content == '4') {
						this.getrandomvideo()
					}
					//获取随机音乐
					else if (this.content == '音乐' || this.content == '5') {
						this.getrandommusic()
					}
					//获取笑话
					else if (this.content == '笑话' || this.content == '8') {
						this.getrandomjok()
					}
					//停止音乐
					else if (this.content == '暂停音乐' || this.content == '6') {
						this.stopmusic()
					}
					//播放音乐
					else if (this.content == '播放音乐' || this.content == '7') {
						this.startmusic()
					}
					//使用帮助
					else if (this.content == '帮助') {
						this.chathelp()
					}
					//切换模式为闲聊
					else if (this.content == '切换为闲聊' || this.content == '1') {
						this.changechattype()
					}
					//切换模式为翻译
					else if (this.content == '切换为翻译' || this.content == '2') {
						this.changechattype()
					}
					//其它内容
					else {
