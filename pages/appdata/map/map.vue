<template>
	<view class="charts-box">
		...
		<view class="search">
			<input type="text" v-model="searchval" placeholder="输入城市名或省名">
		...
		data() {
			return {
				...
				arr: ["甘肃省", "黑龙江省", "吉林省", "辽宁省",
					...
					"北京市", "上海市", "天津市", "重庆市", "武威市", "中国", '昭通市', '昆明市', '香港特                       别行政区', '澳门特别行政区'
				],
				range: [{
						value: 34,
						text: "中国"
		...
		methods: {
			async search() {
				if (!this.searchval) {
					...
					let res = await this.$lw(
						`https://restapi.amap.com/v3/geocode/geo?address=${this.searchval}&key=a5fdf4894007191f11d86303ee5811a7`
					)
					console.log(res, '获取编码')
					if (res.data.status == 0) {
						...
						this.searchdata(res.data.geocodes[0].adcode, res.data.geocodes[0].formatted_address)
					}
				}
			},
			change(e) {
			...
			async searchdata(code, province) {
				console.log(code, '编码')
				let res = await this.$lw('https://geo.datav.aliyun.com/areas_v3/bound/' + code + '_full.json')
				// console.log(res.data.features, '搜索')
				//模拟从服务器获取数据时的延时
				setTimeout(() => {
