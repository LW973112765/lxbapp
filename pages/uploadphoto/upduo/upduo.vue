<template>
	<view class="tui-container">
		<view class="tui-upload-box">
			<view @longpress="updatatitle(index)" class="tui-image-item" v-for="(item,index) in imageList" :key="index">
	...
	<button type="default" style="margin-top: 30rpx;" @click="upphoto">上传</button>
	<view>
		<!-- 输入框示例 -->
		 ...
		 upphoto(){
			 ...
			 var total=this.imageList.length
				this.imageList.forEach((item,index)=>{
					...
					uni.request({
						url:'http://60.204.219.215/api/api/addphoto',
			...
			updatatitle(index){
				...
				console.log(index)
				this.introduction=`您正在为第${index+1}张图片添加描述`
			},
			inputDialogToggle() {
				...
				uni.chooseImage({
					...
					success: function(e) {
						...
						for (let i = 0; i < e.tempFilePaths.length; i++) {
							...
							let path = {url:e.tempFilePaths[i],title:''}
