<template>
	<view class="main">
		...
		<!-- 文章 -->
		<scroll-view scroll-y="true" class="article" refresher-default-style="black"
			v-if="navId==1&&articledata.length>0">
			<ArticleItem :article="articledata" v-for="(item, id) in articledata" :key="item.id" :item="item" />
		</scroll-view>
		<!-- 视频 -->
		<scroll-view class="main" scroll-y="true" enable-flex refresher-default-style="black"
	...
	export default {
		...
		methods: {
			...
			changeNav(id) {
				...
				if (this.navId == 1 && this.searchval) {
					this.getarticle(1, this.searchval)
				} else {
					this.articledata = []
				}
				if (this.navId == 2 && this.searchval) {
					this.getvideo(1, this.searchval)
				} else {
					this.videodata = []
				}
			...
			async getarticle(page, content) {
				let res = await this.$zxl('/getsearch', {
					search: content,
					page: page
				})
			...
			async getvideo(page, content) {
				let res = await this.$zxl('/searchvideo', {
					search: content,
					page: page
				})
			...
			changeColor(articledata1) {
				articledata1.map((item, index) => {
					if (this.searchval) {
						...
						articledata1[index].created_at = item.created_at.replace(
							replaceReg,
							replaceString
						);
					}
...
<style>
	page {
		background: linear-gradient(to right, rgb(66, 83, 216), aqua);
	}
</style>
