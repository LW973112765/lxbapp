<template>
	<view>
		<view class="uni-container">
					...
					<uni-td>
						<view class="uni-group">
							  <button
							              size="mini"
							              type="warn"
							              @click="handleDelete(item.index, item)"
							              >删除</button
							            >
							            ...
							            <button
							              ...
							              >审核通过</button
							            >
							            <button v-if="item.status === 1" type="primary" size="mini"
							              >已通过</button
							            >
							            <button
							              v-if="item.status === 0"
							              ...
							              @click="isDel(2, item)"
							              >拒绝</button
							            ><button v-if="item.status === 2" size="mini" type="primary"
							              >已拒绝</button
							            >
						</view>
					</uni-td>
			...
			<view class="uni-pagination-box"><uni-pagination show-icon :total="total" :page-size="pageSize" :current="currentPage"  @change="changePage" /></view>
		</view>
		<view>
			<!-- 提示信息弹窗 -->
  ...
  methods: {
   ...
   async handleDelete(index, row) {
    ...
    async isDel(is_del, item) {
		console.log('；；；')
      await this.$zxl('/editcomment',{ id: item.id, status: is_del },'POST');
