<template>
    <view class="main">
      <view class="content">
        ...
        <view class="qu-content">
          ...
          <!-- <ul v-else class="options-list">
                       <li v-for="(option, optIndex) in item.options" :key="optIndex">
                         <label>
                           <radio required v-if="item.type === 'radio'" :type="item.type" :name="index + 1"
                           @change="radioAnswer($event, optIndex,index)">
                           <checkbox> required v-else :type="item.type" :name="index + 1"
                             @change="checkboxAnswer($event, optIndex,index)">
                           <text>{{ option }}</text>
                         </label>
                       </li>
                     </ul> -->
		<radio-group name="cc" @change="radioAnswer($event, optIndex,index)" v-for="(option, optIndex) in item.options" :key="'new'+optIndex+1">
			<label v-if="item.type === 'radio'">
			<radio   :checked="optIndex === current" />{{ option }}
			</label>
		</radio-group>
				
				
        <checkbox-group @change="checkboxAnswer($event, optIndex,index)" v-for="(option, optIndex) in item.options" :key="optIndex">
        ...
        <footer>
          <text id="submitBtn" @click="submit">提交问卷</text>
        </footer>
