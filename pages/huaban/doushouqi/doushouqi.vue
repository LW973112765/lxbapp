<template>
	<view class="beast">
		...
		<view class="player player-1" :class="[player1.camp]">
			<view class="photo">
				<image src="../../../static/images/doushouqi/player-ai.png"></image>
			</view>
			<view class="HP">
					...
					<template v-for="item in player1.list">
						<uni-badge class="uni-badge-left-margin" :text="isPlay != 0 ? item.count: ''" absolute="leftTop" size="small">
							<view class="chess-item" :class="[item.count == 0 ? 'die': '', player1.camp]">
								<image :src="`../../../static/images/doushouqi/${item.name}.png`" mode=""></image>
							</view>
						</uni-badge>
					</template>
				</view>
				...
				<view class="chess-play">
					<view class="chess-play-row" v-for="item1,index1 in mapList" :key="index1">
						<view class="chess-block" v-for="item2,index2 in item1" :key="index2" @click="onPlayer(item2, index1, index2)">
							...
							>
								<image :src="`../../../static/images/doushouqi/${item2.name}.png`" mode="" v-show="item2.isFilp"></image>
							</view>
						</view>
					</view>
				</view>
		...
		<view class="player player-2" :class="[player2.camp]">
			...
			<view class="photo">
				<image src="../../../static/images/doushouqi/player-1.png"></image>
			</view>
		</view>
		...
		<uni-popup ref="popupRule2" :isMaskClick="false" :maskBackgroundColor="'rgba(0,0,0,.6)'">
			<view class="win">
				<view class="win-box">
					<view class="win-title">
						<text>游戏规则</text>
			...
			<view class="win-btns">
				<button class="win-btn" @click="closerule()">关闭</button>
			</view>
		</uni-popup>
		<!-- 输赢提示框 -->
...
<script>
	import { deepcopyArray } from '../../../common/doushouqi/doushouqi.js'
	export default {
		data() {
			return {
				player1: {							// 玩家1 - 电脑
				...
				chessList: [
					{name: 'long', weight: 7, count: 1, score: 19, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
					{name: 'hu', weight: 6, count: 2, score: 12, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
					{name: 'xiang', weight: 5, count: 2, score: 10, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
					{name: 'xiong', weight: 4, count: 2, score: 8, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
					{name: 'fu', weight: 3, count: 2, score: 5, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
					{name: 'ying', weight: 2, count: 2, score: 3, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
					{name: 'mao', weight: 1, count: 5, score: 1, camp: null, isFilp: false, isShow: true, isSelected: false, location: []},
				],
				mapList: [],						// 地图二维数组
		...
		onNavigationBarButtonTap(e){
		   this.$refs.popupRule2.open()
		},
		methods: {
			closerule(){
				 this.$refs.popupRule2.close()
			},
			// 生成双方所需棋子、棋盘总棋子
			createPlayChess() {
				let arr1 = [];
