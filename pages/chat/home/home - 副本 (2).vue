...
<script>
	...
	export default {
		...
		methods: {
			getinfo(userid){
			let username=this.friends.filter(item=>{
			...
			getusername(userid){
				...
				return username[0].beizhu || username[0].username;
			},
			async getfriend(){
			...
			renderConversations(content){
				...
				//  					this.friends.forEach(zxl=>{
				//  						if(item.userId==zxl.friendid){
				//  							Object.defineProperty(item,'avatar',{
				// 								value:'zxl',
				// 								configurable:true,
				// 								writable:true
				// 							})
				...
				// consoloe.log(this.conversations,'在看着看着')
