	...
	<view>
		<scroll-view class="conversations" scroll-y="true">
			<view v-if="conversations.length > 0">
				<view class="scroll-item"  v-for="(conversation, key) in conversations" :key="key" :class="{zhiding:conversation.top}" @click="navigateToChat(conversation)">
					<view class="item-head">
						<!-- <image :src="conversation.data.avatar" class="head-icon"></image> -->
						<image :src="getinfo(conversation.userId)" class="head-icon"></image>
					...
					<view class="scroll-item_info" >
						<view class="item-info-top">
							...
							<text class="item-info-top_name">{{getusername(conversation.userId)}}</text>
						...
						<view class="item-info-bottom">
							<view class="item-info-bottom-item">
								<view class="item-info-top_content" v-if="!conversation.lastMessage.recalled">
									...
									<text style="color: red;" v-if="conversation.type === 'private'">{{conversation.lastMessage.senderId == currentUser.id? '我': getusername(conversation.userId)}}:</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// console.log(username[0].avatar,'头像')
			 return name;
			},
			getusername(userid){
				let name=''
