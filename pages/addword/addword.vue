			...
			<view class="textarea_from">
				<u--form :model="loginForm" ref="uLoginForm">
					<u-form-item prop="content">
						<u--textarea v-model="loginForm.title" @focus="passwordF_B" @blur="passwordF_B" height=160
							count :maxlength="maxlength" placeholder="请输入内容..." border="bottom"></u--textarea>
					</u-form-item>
					<view class="btn_Box">
						<button type="primary" class="submitBtn" @click="add()">添加</button>
						<button type="warn" class="resetBtn" @click="resetFrom()">重置</button>
					</view>
				</u--form>
			</view>
	...
	export default {
		...
		methods: {
		  ...
		  async	add() {
			   ...
			   let res=await this.$zxl('/addword',{
			   			
			   			title: this.loginForm.title,
			   			
			   		},'POST')
					if (res.data.status==200) {
						this.messageToggle('success','添加成功')
