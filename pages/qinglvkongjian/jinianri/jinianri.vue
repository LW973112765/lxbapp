<template>
	<view>
		<qinglvkongjianbar @navbarBtnTap="navbarBtnTapHandler" img="../../../static/images/choujiang/ax1.png"
			:title="'纪念日'"></qinglvkongjianbar>
		<option-list class="main" :itemHeight="itemHeight" :list="list" :threshold="threshold" @edit="edit" @del="del"
			:options="options">
			<template slot-scope="{item ,index}">
				<!-- <view class="item" :style="{ backgroundColor: buttons[index].color }"> -->
				<view class="item">
					<image v-show="item.isyear" class="tixin" src="../../../static/images/choujiang/tx2.png" mode="">
					</image>
					<view class="bj">
			...
			<uni-popup ref="popup" style="z-index: 20;" background-color="#fff">
				<view class="popup-contentzxl" :class="{ 'popup-height': type === 'left' || type === 'right' }">
					<view class="top">
						...
						<image @click="close" style="width: 50rpx;height: 50rpx;"
							src="../../../static/images/choujiang/gb.png" mode=""></image>
					</view>
					<view class="center">
						<view class="item">
							<text>纪念描述</text>
							<input placeholder="一个特别的日子◕‿◕" maxlength="15" :cursorSpacing="cursorSpacing" class="des"
								:class="{focus:focus==true}" @focus="focus=true" @blur="focus=false" type="text"
								v-model="title">
							<zero-switch v-show="0" style="margin-left: 25rpx;" class='inline_item' :showText="true"
								backgroundColorOff="#999" backgroundColorOn="#ABDCFF" activeColor='#0396FF'
								inactiveColor='#ffffff'>
							</zero-switch>
						</view>
						<view class="item" @click="show = true">
							<text>纪念日期</text>
							<text class="des">{{time}}</text>
							<image style="width: 50rpx;height: 50rpx;" src="../../../static/images/choujiang/jt.png"
								mode=""></image>
						</view>
						<view class="item">
							<text>每年提醒</text>
							...
							<zero-switch @change="changeyear" style="margin-left: 25rpx;" class='inline_item'
								v-model="switchyear" :showText="true" backgroundColorOff="#999"
								backgroundColorOn="#ABDCFF" activeColor='#0396FF' inactiveColor='#ffffff'>
							</zero-switch>
						</view>
						<view class="item">
							<text>是否农历</text>
							...
							<zero-switch @change="changelunar" style="margin-left: 25rpx;" class='inline_item'
								v-model="switchlunar" :showText="true" backgroundColorOff="#999"
								backgroundColorOn="#ABDCFF" activeColor='#0396FF' inactiveColor='#ffffff'>
							</zero-switch>
						</view>
					</view>
		...
		<view>
			<u-datetime-picker ref="datetimePicker" :show="show" @cancel="cancel" @confirm="confirm" v-model="value1"
				mode="datetime" :minDate="946656000000" :maxDate="4102416000000"
...
<script>
	...
	export default {
		...
		methods: {
			...
			confirm(e) {
				this.time = moment(e.value).format('YYYY-MM-DD HH:MM')
				this.show = !this.show
			},
			...
			getword(item) {
				...
				let result = this.$compare(moment().format('YYYY-MM-DD hh:mm:ss'), moment(item.time).format(
					'YYYY-MM-DD hh:mm:ss'))
					...
					if (item.isyear) {
						...
						if (moment(item.time).format("YYYY") == moment().format("YYYY")) {
							...
							//日期大于当前
								...
								let z = this.$time(moment().format('YYYY-MM-DD hh:mm:ss'), moment(item.time).format(
									'YYYY-MM-DD hh:mm:ss'))
								if (result) {
									return z + '前'
								} else {
						...
						//年份小于当前
							...
							if ((m * 1000 + d * 1) < (M * 1000 + D * 1)) {
								...
								let z = this.$time(moment().format('YYYY-MM-DD hh:mm:ss'), moment(year).format(
									'YYYY-MM-DD hh:mm:ss'))
								return z + '后'
							}
							//日期大于当前
								...
								let m = this.$time(moment().format('YYYY-MM-DD hh:mm:ss'), moment(Year).format(
									'YYYY-MM-DD hh:mm:ss'))
								return m + '后'
							}
			...
			async deljinianri(id) {
				let res = await this.$zxl('/deljinianri', {
					id
				})
				if (res.data.status == 200) {
					this.$refs.uToast.show({
