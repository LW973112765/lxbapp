	...
	<view>
		...
		<view>
			...
			<uni-popup ref="alertDialog" type="dialog">
				<uni-popup-dialog :type="msgType" cancelText="取消" confirmText="确定" title="提示" :content="content"
...
<script>
	...
	export default {
		...
		onLoad() {
			uni.$on('delitem', (id, content) => {
				console.log(id, '收到')
				console.log(content, '收到')
				this.delid = id
				this.dialogToggle('warn', content)
				// this.del(id)
			})
		},
		methods: {
			...
			dialogToggle(type, content) {
				this.msgType = type
				this.$refs.alertDialog.open()
				this.content = `确定要删除【${content}】吗？`
			},
			dialogConfirm() {
				this.del(this.delid)
			},
