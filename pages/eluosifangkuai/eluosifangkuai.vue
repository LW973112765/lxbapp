	...
	<view class="container">
		...
		<view class="top">
			...
			<view class="game-info">
				...
				<view class="game-btn" @tap="continueGame" v-if="!ended && pause">
					<span>继续游戏</span>
				</view>
			</view>
		</view>
		<view class="control">
			<view class="direction">
				<view class="control-btn control-top" @tap="changeBlock">旋转</view>
				<view class="control-btn control-left" @touchstart="leftlongpress" @touchend="left">左移</view>
				<view class="control-btn control-right" @touchstart="rightlongpress" @touchend="right">右移</view>
				<view class="control-btn control-bottom" @touchstart="droplongpress" @touchend="drop">下移</view>
			</view>
			<view class="circle-btn" @tap="quickDrop">快速降落</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			//最高分数
			hiScore () {
				let highscore= uni.getStorageSync("highscore");
				if (highscore) {
			...
			down (isQuick = false) {
				...
				if ( this.error ) {//方块超出界面游戏失败
					...
					if ( this.score > this.hiScore ) {
						uni.setStorageSync('highscore',JSON.stringify(this.score))
					}
					uni.showModal({
						title: '提示',
						content: this.score > this.hiScore ? `游戏结束,您的得分是${this.score},恭喜您刷新最高得分记录。` :`游戏结束,您的得分是${this.score}。`,
						showCancel: false,
						success: (res) => {
							if (res.confirm) {
							   this.endGame();
