	...
	<view class="tui-container">
		...
		<view class="tui-upload-box">
			<view class="tui-image-item" v-for="(item,index) in imageList" :key="index">
				<image :src="item.url" class="tui-item-img" mode="aspectFill" @click="yulan(item,index)"></image>
				<view v-if="!forbidDel" class="tui-img-del" @tap.stop="delImage(index)">
				</view>
		...
		methods: {
			...
			upphoto() {
				...
				this.dataList.forEach((item, index) => {
					this.count++
					uni.request({
						url: 'http://60.204.219.215/api/api/addcover',
						data: {
							url: item.url,
							userid: this.userid
						},
