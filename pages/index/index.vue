<template>
	<view class="indexcontainer" :class="{popupShow:popupShowLeft||popupShowRight}" @touchstart="handleTouchStart"
		...
		<view class="screen-swiper-box">
			<swiper class="lunbo" indicator-dots :autoplay="true" :interval="duration" circular
				indicator-active-color="aqua" indicator-color="#333">
			...
			<view class="popup-content-left" :class="{ 'popup-height': type === 'left' || type === 'right' }">
				<view class="main">
					...
					<view class="box">
						...
						<view class="item" v-if="userinfo.id" @click="luobotugl">
							<image src="../../static/images/other/lbtgl.png" mode=""></image>
							<view>云端轮播图</view>
						</view>
						<view class="item" @click="luobotusetting">
							<image src="../../static/images/other/lbt.png" mode=""></image>
							<view>本地轮播图</view>
						</view>
						<view class="item" @click="toersoninfo" v-if="userinfo.id">
						...
						<view class="item" @click="totranslate">
						...
						<view class="item" @click="tobeiwanglu" v-if="userinfo.id">
							<image src="../../static/images/personal/bianqian.png" mode=""></image>
							<view>备忘录</view>
						</view>
						<view class="item" @click="tojishiben" v-if="userinfo.id">
						...
						<view class="item" @click="tocollect" v-if="userinfo.id">
						...
						<view class="item" @click="toquestion" v-if="userinfo.id">
							<image src="../../static/images/tools/555.png" mode=""></image>
							<view>入站问答</view>
						</view>
						<view class="item" @click="tomessage" v-if="userinfo.id">
							<image src="../../static/images/tools/yijianfankui.png" mode=""></image>
							<view>意见反馈</view>
						</view>
						<view class="item" @click="tonote(userinfo.id)" v-if="userinfo.id">
		...
		methods: {
			async getcover() {
				...
				if (res.data.status == 200) {
					...
					this.testarr = res.data.data.filter(i => i.isused)
					this.tabList = res.data.data.filter(i => i.isused).sort((a, b) => a.time - b.time)

					// this.tabList = this.tabList.length > 0 ? this.tabList : this.coverlist
				} else {
					this.tabList = []
				}
			...
			luobotugl() {
				uni.navigateTo({
					url: '/pages/lunbotumanage/lunbotumanage',
					animationType: 'zoom-fade-out'
				})
			},
			towangzhe() {
