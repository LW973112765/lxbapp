	...
	<view :class="modal?'show-qrcode':'hide-qrcode'">
		<view class="box-qrcode" :style="{'margin-left':  marginLeft + 'rpx'}" @longtap="longtapCode">
			...
			<canvas class="canvas-qrcode" :style="style_w_h" :canvas-id="qrcode_id">
				...
				<!-- #ifndef MP -->
				<!-- 	<view v-if="is_themeImg" :style="style_w_h" class="box-img-qrcode">
					<image style="width: 200rpx;height: 200rpx;" :style="style_w_h_img" mode="scaleToFill"
						:src="themeImg"></image>
				</view> -->
				<view v-if="is_themeImg" class="box-img-qrcode">
					<image mode="scaleToFill" :src="themeImg"></image>
				</view>
				<!-- #endif -->
	...
	export default {
		...
		methods: {
			...
			crtQrCode() {
				...
				// #endif
				//#ifdef MP
				that.createQrCode(this.urll, that.qrcode_id, that.width, that.height, that.themeColor, that.is_themeImg,
					that.themeImg, that.h_w_img);
				// #endif

				//that.createQrCode(this.urll, that.qrcode_id, that.width, that.height);
			},
			//#ifdef MP

			createQrCode: function(url, canvasId, cavW, cavH, cavColor, haveImg, imgurl, imgsize) {
				//调用插件中的draw方法，绘制二维码图片
				qr_we.api.draw(url, canvasId, cavW, cavH, cavColor, haveImg, imgurl, imgsize, this, this
					.canvasToTempImage);
