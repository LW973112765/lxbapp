<template>
	<view class="wyb-pagination-box" :style="{
		...
		<view class="wyb-pagination-left" :style="{opacity: currentPage === 1 ? 0.5 : 1}">
			<view v-if="showFirst" :class="'wyb-pagination-first-page-' + (showIcon ? 'i' : 't')" :style="btnStyleStr"
				:hover-class="currentPage === 1 ? '' : 'wyb-pagination-hover'" @tap="onPageBtnTap('first-page')">
				<view v-if="showIcon" class="iconfont icon-shuangjiantou left-arrow" />
				<text v-else>{{firstText}}</text>
			</view>
			<view :class="'wyb-pagination-prev-page-' + (showIcon ? 'i' : 't')" :style="btnStyleStr"
				:hover-class="currentPage === 1 ? '' : 'wyb-pagination-hover'" @tap="onPageBtnTap('prev-page')">
		...
		<view class="wyb-pagination-info" @tap.stop="onInfoTap">
			<view class="wyb-pagination-num" v-if="!infoClick">
				...
				<text v-if="showTotalItem" class="wyb-pagination-info-total"
			...
			<view class="wyb-pagination-input" v-else>
				<input type="number" v-model="inputPage" :onpaste="false" :focus="infoFocus" :value="currentPage"
					:style="{color: currentColor}" :cursor-spacing="cursorSpacing" @confirm="onInfoConfirm"
					@blur="onInfoBlur" />
			</view>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN || APP-VUE || APP-NVUE || APP-PLUS || APP-PLUS-NVUE -->
			<view class="wyb-pagination-input" v-else>
				<input type="number" v-model="inputPage" :focus="infoFocus" :name="currentPage"
					:style="{color: currentColor}" :cursor-spacing="cursorSpacing" @confirm="onInfoConfirm"
					@blur="onInfoBlur" />
			</view>
			<!-- #endif -->
		...
		<view class="wyb-pagination-right" :style="{opacity: currentPage === totalPage ? 0.5 : 1}">
			<view :class="'wyb-pagination-next-page-' + (showIcon ? 'i' : 't')" :style="btnStyleStr"
				:hover-class="currentPage === totalPage ? '' : 'wyb-pagination-hover'" @tap="onPageBtnTap('next-page')">
				<view v-if="showIcon" class="iconfont icon-danjiantou right-arrow" />
				<text v-else>{{nextText}}</text>
			</view>
			<view v-if="showLast" :class="'wyb-pagination-last-page-' + (showIcon ? 'i' : 't')" :style="btnStyleStr"
				:hover-class="currentPage === totalPage ? '' : 'wyb-pagination-hover'" @tap="onPageBtnTap('last-page')">
